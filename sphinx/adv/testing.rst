...
.. code-block:: python
    ...
    class MatrixCase(unittest.TestCase):
        ...
        def testInvalidType(self):
            self.assertRaises(TypeError, euler, 'text')

        def testInvalidTypeWithCM(self):
...
Biblioteka :mod:`unittest` może uruchamiać wybrane testy za pomocą linii poleceń:

.. code-block:: bash

    python -m unittest module_name1 module_name2
    python -m unittest module_name.test_case
    python -m unittest module_name.test_case.particular_test

Można również wymusić dodatkowe informacje:
